...
def get_sentiment_data():
    """
    Get MyFXBook sentiment data and return as DataFrame
    
    Returns:
        pd.DataFrame: Sentiment data for currency pairs
    """
    try:
        soup = get_soup_from_url("https://www.myfxbook.com/community/outlook")
        rows = soup.find_all('tr', class_='outlook-symbol-row')
        ...
        for r in rows:
            try:
                data = get_data_from_row(r)
                if data and data.get('pair_name'):  # Only add valid data
