...
import src.constants.defs as defs

from dateutil import parser
from datetime import datetime as dt
from src.infrastructure.instrument_collection import instrumentCollection as ic
from src.models.api_price import ApiPrice
from src.models.open_trade import OpenTrade
...
class OandaApi:
    ...
    def validate_credentials(self):
        """Test if the API credentials are valid by making a simple request"""
        try:
            ok, data = self.make_request("accounts")
            if ok:
                print("✅ OANDA API credentials are valid")
                return True
            else:
                print(f"❌ OANDA API credentials are invalid: {data}")
                return False
        except Exception as e:
            print(f"❌ Error validating OANDA API credentials: {e}")
            return False
